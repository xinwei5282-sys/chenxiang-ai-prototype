...
import { BottomSheet, Carousel, FlowStack, KeyboardTextarea, MobileScroll, useKeyboard, useKeyboardInsets, useMobileDevice, type FlowScreen } from "./mobile";
...
const pagePrdDefaults: Record<ProductDocPageKey, PagePrd> = {
  ...
  certificate: { goal: "展示当前或所选手串的数字身份、核验状态、产区、香韵和完整溯源时间线。", roles: "持有效扫描会话的匿名用户；拥有绑定关系的已授权用户", entry: "首页查看证书与溯源；我的手串选择后；手串档案", permission: "公开扫描证书可读；绑定列表来源需校验账号归属", rules: ["首页使用 current_bracelet_id，列表使用 selected_bracelet_id。", "名称、证书编号、产区、香韵和时间线必须来自同一手串记录。", "详情页只读，不增加相邻证书左右切换。"], fields: [["手串名称", "文本", "必显", "与 bracelet_id 对应"],["证书编号", "文本", "必显", "唯一证书标识"],["核验状态", "状态标签", "必显", "展示已核验、待核验、异常或已撤销"],["产区与香韵", "文本", "有数据展示", "字段缺失显示信息待补充"],["溯源时间线", "时间线", "有数据展示", "展示原料批次、制作、质检和芯片绑定节点"]], actions: [["返回", "始终展示", "返回首页、手串列表或档案来源", "保留来源上下文"],["滚动查看时间线", "时间线存在", "查看完整溯源节点", "只记录页面曝光，不修改证书"]], boundary: "证书不得被表述为材质功效、价格或投资价值的绝对承诺。", states: "加载→证书有效/信息缺失/不存在/已撤销。", logs: "记录 bracelet_id、certificate_id、查看来源和核验结果。", empty: "字段缺失显示信息待补充；证书不存在或撤销时不得显示已核验。", acceptance: ["从不同手串进入时名称、编号和时间线一致对应。", "首页仍直达当前手串，Profile 仍先进入手串列表。"], tech: ["证书数据需防篡改校验。", "current 与 selected 手串上下文必须隔离。"] },
  ...
  "certificate-list": { goal: "让多手串用户先选择手串，再查看所选手串的证书与溯源。", roles: "拥有绑定手串的已授权消费者", entry: "我的→证书数量", permission: "微信授权后，仅本人绑定数据", rules: ["导航标题下直接开始全宽列表，不显示眉题、大标题、数量说明或引导文案。", "点击任一手串整行直接进入该手串的证书与溯源详情。"], fields: [["缩略图", "图片", "每行必显", "与 bracelet_id 对应"],["手串名称", "文本", "每行必显", "使用绑定记录名称"],["证书编号", "文本", "每行必显", "原型三条编号唯一"],["规格", "文本", "有数据展示", "展示珠径和颗数"],["绑定状态", "状态文本", "每行必显", "当前原型统一显示已绑定"]], actions: [["手串列表项", "有绑定数据", "进入所选手串的证书与溯源详情", "记录 bracelet_id 并校验归属"],["返回", "始终展示", "回到我的", "保留列表位置"]], boundary: "只展示当前账号已绑定手串；不在详情内增加左右滑动或相邻证书切换。", states: "加载→手串列表/空态/失败；选择手串→对应证书详情。", logs: "记录列表查看、所选 bracelet_id 和加载错误。", empty: "无绑定手串时展示空态和芯片识别说明；失败提供重试。", acceptance: ["原型恰好展示三条且编号唯一。", "点击第二、第三条后详情不得显示第一条数据。", "列表行触控高度不小于 76px。"], tech: ["列表接口需支持分页、绑定状态和账号隔离。", "证书详情必须使用所选 bracelet_id 查询。"] },
